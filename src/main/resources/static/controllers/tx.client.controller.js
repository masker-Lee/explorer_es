...
//    };
    ...
    stompClient.connect({}, function(frame) {//链接Web Socket的服务端。
        stompClient.subscribe('/ws/transaction', function(respnose){ //订阅/topic/getResponse 目标发送的消息。这个是在控制器的@SendTo中定义的。
            if(respnose.body) $scope.addTX();
        });
    });
...
function UnconfirmedTXController($scope, $timeout, $interval, $location, TXService){
	...
	var socket1 = new SockJS('/endpointWisely'); //链接SockJS 的endpoint 名称为"/endpointWisely"
    stompClient = Stomp.over(socket1);//使用stomp子协议的WebSocket 客户端
    stompClient.connect({}, function(frame) {//链接Web Socket的服务端。
        stompClient.subscribe('/ws/unconfirmed', function(e) {
			if(!e || !e.body)
				return;
