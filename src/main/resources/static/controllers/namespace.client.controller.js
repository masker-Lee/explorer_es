...
function NamespaceListController($scope, $timeout, NamespaceService){
	...
	$scope.showSubs = (index, $event) => {
		...
		$scope.subsRootNamespaceId = $scope.namespaceList[index].id
...
function NamespaceController($scope, $timeout, $location, NamespaceService, MosaicService){
	...
	let rootNamespaceId = $location.search().rootNamespaceId;
	if(!ns || !rootNamespaceId){
		$scope.message = "Search condition [ns or rootNamespaceId] is needed";
		return;
	}
	if (!(/(^[1-9]\d*$)/.test(rootNamespaceId))) { 
	　　　$scope.message = "rootNamespaceId error";
	　　　return; 
	}
	$scope.searchNamespace = ns;
	$scope.rootNamespaceId = rootNamespaceId;
	$scope.searchSub = "";
	if(ns.indexOf(".")!=-1){
		$scope.searchNamespace = ns.substring(0, ns.indexOf("."));
		$scope.searchSub = ns;
	}
	let params = {ns: $scope.searchNamespace, rootNamespaceId: $scope.rootNamespaceId};
